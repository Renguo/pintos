...
struct cache_block
{
  ...
  int read_waiters;          /* number of waiting to read. */
  int writers;
  int write_waiters;
...
/*
 * initialize the all block cache and variable inside
 */
void
cache_init (void)
{
...
cache_flush (void)
{
  ...
  for (i = 0; i < CACHE_CNT; i++)
  {
    struct cache_block *b = &cache[i];
    block_sector_t sector;
    // lock cache block prevent read and write
...
/* Locks the given SECTOR into the cache and returns the cache block.
 * if cache already has this sector, return it.
 * if cache doesn't have this sector, cache it from disk to an unused cache_block
 * if there is no unused cache_block, evict an unlocked one
 * if every cache_block in use, wait 1000ms then try again.
 */
struct cache_block *
...
try_again:
  ...
  for (i = 0; i < CACHE_CNT; i++)
  {
     ...
     * if lock type is NON_EXCLUSIVE, it means that there might be more than one lockers
     * and it is read lock since we allow multiple reader of a file
     */
    ...
    /* Else lock type is EXCLUSIVE, means that it is a write lock
     * since we only allow one writer at a time
     */
    else
    {
      /* Lock for write. */
  ...
  lock_release (&cache_sync);
  // wait for evicted sector available
  timer_msleep (1000);
  goto try_again;
}
...
flushd (void *aux UNUSED)
{
  for (;;)
  {
    /* flush cache to disk periodically
     */
