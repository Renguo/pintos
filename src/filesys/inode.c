...
#define DOUBLE_INDIRECT_CNT 1
#define SECTOR_CNT (DIRECT_CNT + INDIRECT_CNT + DOUBLE_INDIRECT_CNT)

/* PTRS_PER_SECTOR is 128, represents the maximum addresses a sector can store */
#define PTRS_PER_SECTOR ((off_t) (BLOCK_SECTOR_SIZE / sizeof (block_sector_t)))

/* BLOCK_SECOT_SIZE is 512 */
/* INODE_SPAN is 8M, represents the total memory 1 inode can support */
...
void inode_init (void)
{
  ...
  /* initialize the lock for protecting the open node list. */
  lock_init (&open_inodes_lock);
}

/* Initializes an inode with LENGTH bytes of data and given inode type (file or directory) and
  writes the new inode to sector SECTOR on the file system
  device.
  Returns the in memory inode if successful.
  */
struct inode * inode_create (block_sector_t sector, enum inode_type type)
{
  ...
  /* lock the sector into the cache block. */
  ...
  /* zero out the cache block and assign the pointer to the zeroed
   * data to the newly created disk inode. */
...
inode_open (block_sector_t sector)
{
  ...
  inode->sector = sector;
  inode->open_cnt = 1;
  /* initialize the locks in the newly created inode */
...
/* Returns the type of given inode. We first get the section number of disk location from the inode, then
 * lock this section into the cache, then get disk inode from cache, which contains the inode type. */
enum inode_type
...
inode_close (struct inode *inode)
{
  ...
  if (--inode->open_cnt == 0)
  {
    ...
    /* Deallocate blocks all indirect sector under the inode if removed. */
    if (inode->removed)
...
/* Deallocates SECTOR and anything it points to recursively if it's indirect or doubly indirect. */
static void
deallocate_recursive (block_sector_t sector, int level)
{
  if (level > 0)
  {
      /* lock the sector into cache */
    struct cache_block *block = cache_lock (sector, EXCLUSIVE);
    /* get the pointers stored in the sector from cache block */
    block_sector_t *pointers = cache_read (block);
    int i;
    /* for each sector pointed to from the current sector, recursively free them */
...
deallocate_inode (const struct inode *inode)
{
  ...
  for (i = 0; i < SECTOR_CNT; i++)
    ...
    {
        /* get the level of the sector, 2 if is a double indirect sector,
         * 1 if is indirect sector, 0 if is a direct sector. */
...
calculate_indices (off_t sector_idx, size_t offsets[], size_t *offset_cnt)
{
  ...
  if (sector_idx < DOUBLE_INDIRECT_CNT * PTRS_PER_SECTOR * PTRS_PER_SECTOR)
  {
                ...
                struct cache_block **data_block)
{
  ...
  /* get a sequence of block indexes in offset */
  calculate_indices (offset / BLOCK_SECTOR_SIZE, offsets, &offset_cnt);
  level = 0;
  this_level_sector = inode->sector;
  /* we go from level 0 to 2 until reach the data block which matches the given offset. */
  for (;;)
  {
...
extend_file (struct inode *inode, off_t length)
{
  if (length > inode_length (inode))
  {
    ...
    if (length > disk_inode->length)
    {
      ...
      /* extend the inode length and mark cache dirty so it will be written back to disk. */
 ...
 * Lock the sector into cache block, read the cache data to get inode length. */
off_t
